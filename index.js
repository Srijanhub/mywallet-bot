...
bot.on('message', async (msg) => {
  ...
  if (msg.voice || msg.audio) {
    try {
      ...
      // If we're waiting for a yes/no confirmation, treat the transcript as a text reply
      ...
      if (confirmationStates.includes(session.state)) {
        ...
        const isYes = ['yes', 'yeah', 'yep', 'yup', 'sure', 'ok', 'okay', 'confirm', 'go', 'save', 'do it'].includes(normalized);
        const isNo  = ['no', 'nope', 'nah', 'cancel', 'stop', 'abort', 'nevermind', 'never'].includes(normalized);
        if (isYes || isNo) {
          ...
          return processMessage(chatId, answer, session);
        }
      }
