  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        catch (e) { reject(new Error('Files API parse error: ' + data)); }
      });
    });
    req.on('error', reject);
    req.write(body);
...
async function transcribeVoice(fileBuffer, mimeType = 'audio/ogg') {
  // Telegram voice = ogg/opus. Try inline first, fallback to Files API.
  const tryMimeTypes = [mimeType, 'audio/ogg', 'audio/webm', 'audio/mp4'];

  // Method 1: inline base64 (fast, works for most formats)
  for (const mt of [...new Set(tryMimeTypes)]) {
    try {
      console.log(`[Voice] Trying inline with mimeType: ${mt}`);
      const base64Audio = fileBuffer.toString('base64');
      const result = await model.generateContent([
        { inlineData: { mimeType: mt, data: base64Audio } },
        { text: `Listen to this audio and transcribe exactly what the person says about their expenses/spending/income.
Return ONLY the spoken words, nothing else. No explanations.
If unclear, make your best guess at the transcription.` },
      ]);
      const text = result.response.text().trim();
      if (text && text.length > 2) {
        console.log(`[Voice] Inline transcription succeeded: "${text}"`);
        return text;
      }
    } catch (e) {
      console.log(`[Voice] Inline failed for ${mt}: ${e.message}`);
    }
  }

  // Method 2: Gemini Files API (more reliable for ogg/opus)
  try {
    console.log('[Voice] Trying Files API upload...');
    const uploadResult = await uploadToGeminiFiles(fileBuffer, 'audio/ogg');
    const fileUri = uploadResult?.file?.uri;
    if (!fileUri) throw new Error('No file URI returned: ' + JSON.stringify(uploadResult));

    console.log('[Voice] File uploaded, URI:', fileUri);
    const result = await model.generateContent([
...
If unclear, make your best guess at the transcription.` },
    ...
    const text = result.response.text().trim();
    console.log(`[Voice] Files API transcription: "${text}"`);
    return text || null;
  } catch (e) {
    console.error('[Voice] Files API error:', e.message);
    return null;
  }
}
