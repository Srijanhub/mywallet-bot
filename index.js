...
const genAIBeta  = new GoogleGenerativeAI(GEMINI_KEY);
const model      = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' }, { apiVersion: 'v1' });
// audio needs v1beta for fileData support, but inline base64 works on v1
const audioModel = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' }, { apiVersion: 'v1' });
const audioModelBeta = genAIBeta.getGenerativeModel({ model: 'gemini-1.5-flash' }, { apiVersion: 'v1beta' });
...
async function transcribeVoice(fileBuffer) {
  ...
  try {
    ...
    const result = await audioModelBeta.generateContent([
