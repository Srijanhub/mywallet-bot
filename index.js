...
const ANTHROPIC_KEY    = process.env.ANTHROPIC_KEY || null;
...
function geminiRESTOnce(model, payload) {
  return new Promise((resolve, reject) => {
...
async function geminiREST(model, payload, retries = 3, delayMs = 4000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    const res = await geminiRESTOnce(model, payload);
    if (res.status === 429) {
      if (attempt < retries) {
        const wait = delayMs * attempt;
        console.log(`[Gemini] Rate limited (429), retrying in ${wait}ms... (attempt ${attempt}/${retries})`);
...
// ── Quick regex-based intent detector (no AI needed) ─────────────────────────
function quickParseIntent(text) {
  const lower = text.toLowerCase().trim();
  const now = new Date();
  const todayStr = now.toISOString().split('T')[0];

  // Detect delete intent
  if (/\b(delete|remove|cancel|undo)\b/.test(lower)) {
    return { intent: 'delete', date: todayStr, isDateRange: /\ball\b/.test(lower) };
  }
  // Detect edit intent
  if (/\b(change|edit|update|fix|correct|modify)\b/.test(lower)) {
    return { intent: 'edit', date: todayStr, isDateRange: false };
  }
  // Everything else is a log — but check for a date mention
  let dateObj = null;
  if (/\byesterday\b/.test(lower)) {
    const d = new Date(now); d.setDate(d.getDate() - 1);
    dateObj = d.toISOString().split('T')[0];
  } else {
    // "on June 5", "on june 5th", "on 5 june"
    const monthNames = ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec'];
    const m1 = lower.match(/\bon\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+(\d{1,2})/i);
    const m2 = lower.match(/\bon\s+(\d{1,2})(?:st|nd|rd|th)?\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i);
    if (m1) {
      const mo = monthNames.indexOf(m1[1].toLowerCase().slice(0,3));
      if (mo !== -1) dateObj = `${now.getFullYear()}-${String(mo+1).padStart(2,'0')}-${String(m1[2]).padStart(2,'0')}`;
    } else if (m2) {
      const mo = monthNames.indexOf(m2[2].toLowerCase().slice(0,3));
      if (mo !== -1) dateObj = `${now.getFullYear()}-${String(mo+1).padStart(2,'0')}-${String(m2[1]).padStart(2,'0')}`;
    }
  }
  return { intent: 'log', date: dateObj || todayStr, isDateRange: false };
}

// ── Quick regex expense parser (no AI needed) ─────────────────────────────────
// Handles: "200 food, 80 uber", "food 200, uber 80", "salary 50000", "spent 200 food"
function quickParseExpenses(text, categories) {
  // Strip date phrases so they don't confuse parsing
  const stripped = text
    .replace(/\bon\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+\d{1,2}/gi, '')
    .replace(/\bon\s+\d{1,2}(?:st|nd|rd|th)?\s+(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*/gi, '')
    .replace(/\b(yesterday|today|this week)\b/gi, '')
    .replace(/\b(spent|earned|got|received|paid|added)\b/gi, '')
    .trim();

  // Known income keywords
  const incomeWords = ['salary','sal','income','earning','earnings','received','got','bonus','stipend','freelance','revenue','profit'];
  const catMap = {};
  Object.keys(categories).forEach(c => { catMap[c.toLowerCase()] = c; });

  // Apply autocorrect map
  const autoCorrect = (word) => {
    const w = word.toLowerCase();
    return AUTOCORRECT_MAP[w] || w;
  };

  // Match category from known list (fuzzy: starts-with or contains)
  const matchCategory = (word) => {
    const w = autoCorrect(word).toLowerCase();
    if (catMap[w]) return catMap[w];
    // starts-with match
    const sw = Object.keys(catMap).find(k => k.startsWith(w) || w.startsWith(k));
    if (sw) return catMap[sw];
    return null;
  };
  ...
  for (const part of parts) {
    // Pattern 1: <amount> <category words>  e.g. "200 food", "5000 mutual funds"
    const m1 = part.match(/^(\d+(?:\.\d+)?)\s+(.+)$/);
    // Pattern 2: <category words> <amount>  e.g. "food 200", "uber 80"
    const m2 = part.match(/^(.+?)\s+(\d+(?:\.\d+)?)$/);

    let amount = null, categoryRaw = null;
    if (m1) { amount = parseFloat(m1[1]); categoryRaw = m1[2].trim(); }
    else if (m2) { amount = parseFloat(m2[2]); categoryRaw = m2[1].trim(); }
    else continue;

    if (!amount || !categoryRaw) continue;

    const catMatched = matchCategory(categoryRaw);
    const catFinal   = catMatched || (categoryRaw.charAt(0).toUpperCase() + categoryRaw.slice(1).toLowerCase());
    const isIncome   = incomeWords.some(w => categoryRaw.toLowerCase().includes(w));
    const isNew      = !catMatched;

    results.push({
      type: isIncome ? 'income' : 'expense',
      amount,
      label: `${catFinal} ${isIncome ? 'income' : 'expense'}`,
      category: catFinal,
      isNewCategory: isNew,
      originalWord: null,
      correctedTo: null,
...
Return ONLY the JSON:`;
  ...
  // Fast path: use regex for common cases (no AI quota used)
  const quick = quickParseIntent(text);
  // Only call AI if the message contains complex natural language
  // (AI is needed for relative dates like "last friday", ambiguous phrasing, etc.)
  const needsAI = /\b(last|next|ago|week|month|morning|evening|night|earlier|just now|recently)\b/i.test(text);
  if (!needsAI) {
    console.log('[parseIntent] Used regex, skipping AI');
    return quick;
  }
  try {
    const res = await geminiREST('gemini-2.5-flash', { contents: [{ parts: [{ text: prompt }] }] });
    if (res.status !== 200) return quick;
...
// Handles: "hundred", "two hundred", "five thousand", "one lakh", "two point five"
// Also handles Indian numbering: lakh, crore
const ONES = {
  zero:0,one:1,two:2,three:3,four:4,five:5,six:6,seven:7,eight:8,nine:9,
  ten:10,eleven:11,twelve:12,thirteen:13,fourteen:14,fifteen:15,sixteen:16,
  seventeen:17,eighteen:18,nineteen:19,
  // common voice mishearings of numbers
  won:1,to:2,too:2,fore:4,sex:6,ate:8,niner:9,
  'one':1,'a':1,
};
const TENS = {
...
function wordsToNumber(words) {
  // words is an array of lowercase tokens
  let total = 0;
  let current = 0;
  for (let i = 0; i < words.length; i++) {
    ...
    } else if (MULTIPLIERS[w] !== undefined && w !== 'hundred') {
      current = (current || 1) * MULTIPLIERS[w];
      total += current;
      current = 0;
    } else if (w === 'point' || w === 'dot') {
      // handle decimal: "two hundred point five" → 200.5
      const decimalPart = words[i + 1];
      if (decimalPart && /^\d+$/.test(decimalPart)) {
        total += current + parseFloat('0.' + decimalPart);
        current = 0;
        i++;
      } else if (decimalPart && ONES[decimalPart] !== undefined) {
        total += current + parseFloat('0.' + ONES[decimalPart]);
        current = 0;
        i++;
      }
    } else {
      // not a number word — flush and stop
...
// Convert spoken numbers in a transcript to digits
// "hundred food" → "100 food"
// "two hundred fifty food" → "250 food"
// "five thousand mutual funds" → "5000 mutual funds"
// "one lakh salary" → "100000 salary"
function convertSpokenNumbers(text) {
  // First replace plain digit-like words that are clearly amounts
  // e.g. "hundred" alone at start of phrase → "100"
  ...
  while (i < tokens.length) {
    const tok = tokens[i];
    // Check if this token starts a number word sequence
    if (ONES[tok] !== undefined || TENS[tok] !== undefined || tok === 'hundred' || MULTIPLIERS[tok] !== undefined) {
      // Greedily consume number words
      const numTokens = [];
      let j = i;
      while (j < tokens.length) {
        const t = tokens[j];
        if (ONES[t] !== undefined || TENS[t] !== undefined || t === 'hundred' || MULTIPLIERS[t] !== undefined || t === 'point' || t === 'dot') {
          numTokens.push(t);
          j++;
        } else {
          break;
      ...
      const num = wordsToNumber(numTokens);
      if (num !== null && num > 0) {
...
CRITICAL - Spoken number handling (numbers may still appear as words):
- "hundred" alone = 100
- "two hundred" = 200, "five hundred" = 500
- "thousand" = 1000, "two thousand" = 2000
- "five thousand" = 5000, "fifty thousand" = 50000
- "one lakh" = 100000, "two lakh" = 200000
- "hundred fifty" = 150, "two fifty" = 250
- Always extract a numeric amount even if written as words
...
Each item must have:
- "type": "expense", "income", or "investment" (use investment for: mutual funds, SIP, stocks, shares, crypto, FD, gold, NPS, PPF, bonds)
- "amount": number (required)
NOTE: investments are NOT expenses and do NOT count against budget
...
Return ONLY the JSON array:`;
  ...
  // Fast path: try regex parser first (zero API calls)
  const quickResult = quickParseExpenses(text, categories);
  if (quickResult) {
    console.log('[parseExpenses] Used regex, skipping AI');
    return quickResult;
  }
  console.log('[parseExpenses] Regex failed, falling back to AI');

  // Try Claude first, fall back to Gemini if unavailable
  if (ANTHROPIC_KEY) {
    try {
      const res = await claudeREST(prompt);
      if (res.status !== 200) throw new Error('HTTP ' + res.status);
      const raw   = res.body?.content?.[0]?.text?.trim() || '';
      const clean = raw.replace(/```json|```/g, '').trim();
      console.log('[parseExpenses] Used Claude');
      return JSON.parse(clean);
    } catch (e) {
      console.error('[parseExpenses] Claude failed, trying Gemini:', e.message);
    }
  }
  try {
    const res = await geminiREST('gemini-2.5-flash', { contents: [{ parts: [{ text: prompt }] }] });
...
function formatEntries(entries, categories) {
  return entries.map(e => {
    ...
    const sign = e.type === 'income' ? '+' : e.type === 'investment' ? '↗' : '-';
    const typeTag = e.type === 'investment' ? ' 📈' : '';
...
bot.on('message', async (msg) => {
  ...
  if (msg.voice || msg.audio) {
    try {
      ...
      // Convert spoken numbers to digits BEFORE parsing
      const converted = convertSpokenNumbers(transcript);
      const showConverted = converted !== transcript.toLowerCase()
        ? `\n🔢 *Numbers converted:* _"${converted}"_` : '';

      await bot.sendMessage(chatId,
        `🎤 *I heard:*\n_"${transcript}"_${showConverted}\n\nProcessing this now...`,
        { parse_mode: 'Markdown' }
      );
      session.lastTranscription = converted;
      ...
      return processMessage(chatId, converted, session);
    } catch (e) {
  ...
  if (text === '/summary') {
    try {
      ...
      const balance = income - expense; // investments don't affect balance
