...
async function transcribeVoice(fileBuffer, mimeType = 'audio/ogg') {
  try {
    ...
    const result = await model.generateContent([
      ...
      {
        text: `This is a voice message about personal expenses. 
Transcribe exactly what the person said, word for word.
If you cannot understand the audio clearly, still try to transcribe what you hear.
...
bot.on('message', async (msg) => {
  ...
  if (msg.voice || msg.audio) {
    try {
      ...
      if (!transcript) {
        return bot.sendMessage(chatId,
          `❌ Couldn't transcribe the voice message. Please try again or type your expenses.`
        );
      }
